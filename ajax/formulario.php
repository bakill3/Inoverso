...
$detalhes = json_decode(file_get_contents("http://ipinfo.io/{$ip}")); //API para ir buscar o país onde o utilizador se encontra
$pais = $detalhes->country; // -> País em que o utilizador se encontra;
...
$nome_do_negocio = htmlspecialchars(mysqli_real_escape_string($link, $_POST['nome_do_negocio']));
$website = htmlspecialchars(mysqli_real_escape_string($link, $_POST['website']));
$cidade = htmlspecialchars(mysqli_real_escape_string($link, $_POST['cidade']));
$descricao = htmlspecialchars(mysqli_real_escape_string($link, $_POST['descricao']));
$gasto = htmlspecialchars(mysqli_real_escape_string($link, $_POST['gasto']));
$data = htmlspecialchars(mysqli_real_escape_string($link, $_POST['data']));
$telefone = htmlspecialchars(mysqli_real_escape_string($link, $_POST['telefone']));
$data_para_inserir = strtotime($data);
$data_para_inserir = date('Y-m-d', $data_para_inserir);


$digitos = 6;
$pass_gerada = rand(pow(10, $digitos-1), pow(10, $digitos)-1);
$password = password_hash($pass_gerada , PASSWORD_DEFAULT);


if (!empty($nome) && !empty($apelido) && !empty($email) && !empty($nome_do_negocio) && !empty($website) && !empty($cidade) && !empty($descricao) && !empty($gasto)) {

	$check_user = mysqli_query($link, "SELECT id FROM users WHERE email='$email'");
	$conta_check = mysqli_num_rows($check_user);
	if ($conta_check == 0) {
		if (mysqli_query($link, "INSERT INTO users(nome, apelido, email, password, data_nascimento, telefone) VALUES ('$nome', '$apelido', '$email', '$password', '$data_para_inserir', '$telefone')")) {
			$id_user = mysqli_insert_id($link);
			$_SESSION['user'][0] = $id_user;
			$_SESSION['mensagem'] = '<p>O seu projeto foi enviado. Foi criada uma conta, onde pode acompanhar o seu projeto em tempo real.<br>Email: '.$email.'<br>Password Gerada: '.$pass_gerada.'<br><br>Detalhes do seu projeto:<br>Nome: '.$nome.'<br>Apelido: '.$apelido.'<br>Cidade: '.$cidade.'<br>Descrição: '.$descricao.'<br>Quanto pretende gastar: '.$gasto.'<br>Telefone: '.$telefone.'<br>Data de Nascimento: '.$data.'';
		}
	} else {
		$info_check = mysqli_fetch_assoc($check_user);
		$id_user = $info_check['id'];
		$_SESSION['user'][0] = $id_user;
		$_SESSION['mensagem'] = '<p>O seu projeto foi enviado. Pode utilizar a conta que já tinha criado para acompanhar o seu projeto em tempo real.<br>Email: '.$email.'<br>Detalhes do seu projeto:<br>Nome: '.$nome.'<br>Apelido: '.$apelido.'<br>Cidade: '.$cidade.'<br>Descrição: '.$descricao.'<br>Quanto pretende gastar: '.$gasto.'<br>Telefone: '.$telefone.'<br>Data de Nascimento: '.$data.'';
	}

	if (mysqli_query($link, "INSERT INTO localizacoes(pais, cidade) VALUES('$pais', '$cidade')")) {
		$id_localizacao = mysqli_insert_id($link);
	}

	if (mysqli_query($link, "INSERT INTO orcamentos(orcamento_final) VALUES('$gasto')")) {
		$id_orcamento = mysqli_insert_id($link);
	}

	if (mysqli_query($link, "INSERT INTO projetos(id_user, nome_do_negocio, descricao, id_orcamento, id_localizacao, website) VALUES((SELECT id FROM users WHERE id='$id_user'), '$nome_do_negocio', '$descricao', (SELECT id_orcamento FROM orcamentos WHERE id_orcamento='$id_orcamento'), (SELECT id_localizacao FROM localizacoes WHERE id_localizacao='$id_localizacao'), '$website')")) {
		$id_projeto = mysqli_insert_id($link);
		$_SESSION['user'][1] = $id_projeto;
		$_SESSION['user'][2] = $email;
	}
    ...
    $mail->addAddress('gabrielvicent98@gmail.com');
		...
		//$bodyContent = '<h1>Projeto enviado com sucesso</h1>';
...
<!--[if mso]></td></tr></table><![endif]-->
	...
	<div style='color:#555555;font-family:'Cabin', Arial, 'Helvetica Neue', Helvetica, sans-serif;line-height:120%; padding-right: 10px; padding-left: 10px; padding-top: 20px; padding-bottom: 10px;'>	
		<div style='line-height:14px;font-size:12px;color:#555555;font-family:'Cabin', Arial, 'Helvetica Neue', Helvetica, sans-serif;text-align:left;'><p style='margin: 0;line-height: 14px;font-size: 12px'><span style='font-size: 16px; line-height: 19px;'>".$_SESSION['mensagem']."</span></p></div>	
	</div>
	<!--[if mso]></td></tr></table><![endif]-->
</div>
...
<table border='0' cellpadding='0' cellspacing='0' width='100%' class='divider ' style='border-collapse: collapse;table-layout: fixed;border-spacing: 0;mso-table-lspace: 0pt;mso-table-rspace: 0pt;vertical-align: top;min-width: 100%;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%'>
  ...
  <!--[if (mso)|(IE)]></div><![endif]-->
		...
		$_SESSION['data_ajax'] = array();
		$_SESSION['data_ajax'][0] = $nome;
		$_SESSION['data_ajax'][1] = $apelido;
		$_SESSION['data_ajax'][2] = $email;
		$_SESSION['data_ajax'][3] = $cidade;
		$_SESSION['data_ajax'][4] = $pass_gerada;
		$_SESSION['data_ajax'][5] = $descricao;
		$_SESSION['data_ajax'][6] = $gasto;
    $_SESSION['data_ajax'][7] = $data;
    $_SESSION['data_ajax'][8] = $telefone;

	}

		//echo "Projeto enviado! Foi-lhe enviado um email com mais detalhes";
}
?>