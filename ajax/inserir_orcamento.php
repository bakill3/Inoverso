...
$id_cliente = htmlspecialchars(mysqli_real_escape_string($link, $_POST['id_cliente']));
$servico = htmlspecialchars(mysqli_real_escape_string($link, $_POST['servico']));
...
$id_user = htmlspecialchars(mysqli_real_escape_string($link, $_POST['id_user']));


$data = $_POST['data_acabamento'];

$data_para_inserir = date('Y-m-d H:i:s', strtotime($data));

if ($id_cliente == 63) {
	$admin = "Gabriel Brandão";
} else {
	$admin = "Gabriel Vicente";
}
$descricao .= "<br>Serviço: ".$servico."<br>Administrador escolhido:".$admin;

$query_proj = mysqli_query($link, "SELECT id_localizacao, id_orcamento FROM projetos WHERE id_user='$id_user'") or die(mysqli_error($link));
$info_proj = mysqli_fetch_assoc($query_proj);
$id_localizacao = $info_proj['id_localizacao'];
$id_orcamento = $info_proj['id_orcamento'];

//$id_orcamento = mysqli_insert_id($con);

if (!empty($servico) && !empty($titulo) && !empty($descricao)) {
	if (isset($_FILES["imagem_upload"]["tmp_name"])) {

		$existe = "../anexos/utilizadores/".$id_user."";
		//$existe = "../imagens/portfolio/".$id_projeto."/main";
		if (!is_dir($existe)) {
			mkdir(''.$existe.'', 0777, true);
		}
		$targetPath = "anexos/utilizadores/".$id_user."/".$_FILES["imagem_upload"]["name"]; 

		$caminho = "../anexos/utilizadores/".$id_user."/".$_FILES["imagem_upload"]["name"]; 

		move_uploaded_file($_FILES["imagem_upload"]["tmp_name"], $caminho);    

		//$query .= ", imagem='$targetPath'";
		//(SELECT id_localizacao FROM localizacoes WHERE id_localizacao='$id_localizacao')
		//(SELECT id_orcamento FROM orcamentos WHERE id_orcamento='$id_orcamento')

		$query = "INSERT INTO projetos(nome_do_negocio, descricao, id_orcamento, id_localizacao, data_acabamento, id_user, anexo) VALUES ('$titulo', '$descricao', (SELECT id_orcamento FROM orcamentos WHERE id_orcamento='77'), (SELECT id_localizacao FROM localizacoes WHERE id_localizacao='$id_localizacao'), '$data_para_inserir', '$id_user', '$targetPath')";


	} else {
		$query = "INSERT INTO projetos(nome_do_negocio, descricao, id_orcamento, id_localizacao, data_acabamento, id_user) VALUES ('$titulo', '$descricao', (SELECT id_orcamento FROM orcamentos WHERE id_orcamento='77'), (SELECT id_localizacao FROM localizacoes WHERE id_localizacao='$id_localizacao'), '$data_para_inserir', '$id_user')";
	}
	...
	//ENVIAR EMAIL ------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		$mail->addAddress($_SESSION['login'][0]);   // Add a recipient
    $mail->addAddress('gabrielvicent98@gmail.com');
		...
		* {
		...
		<div style='font-size:12px;line-height:14px;color:#132F40;font-family:'Cabin', Arial, 'Helvetica Neue', Helvetica, sans-serif;text-align:left;'><p style='margin: 0;font-size: 14px;line-height: 17px'><span style='font-size: 22px; line-height: 26px;'>Olá&#160;<strong>".$_SESSION['login'][2]." ".$_SESSION['login'][3]."</strong>, projeto enviado!</span></p></div>	
		</div>
		...
		<div style='line-height:14px;font-size:12px;color:#555555;font-family:'Cabin', Arial, 'Helvetica Neue', Helvetica, sans-serif;text-align:left;'><p style='margin: 0;line-height: 14px;font-size: 12px'><span style='font-size: 16px; line-height: 19px;'><p>O seu projeto foi enviado. Pode utilizar a conta que já tinha criado para acompanhar o seu projeto em tempo real.<br>Email: ".$_SESSION['login'][0]."<br>Detalhes do seu projeto:<br>Nome: ".$_SESSION['login'][2]."<br>Apelido: ".$_SESSION['login'][3]."<br>Título: ".$titulo."<br>Descrição: ".$descricao."<br>Data Limite: ".$data."</span></p></div>	
		</div>
		<!--[if mso]></td></tr></table><![endif]-->
		</div>
	...
	echo json_encode(array('status' => '1', 'mensagem' => 'Orçamento enviado'));
} else {
	echo json_encode(array('status' => '0', 'mensagem' => 'Preencha todos os dados'));
}
?>